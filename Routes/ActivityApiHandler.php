...
class ActivityApiHandler implements ApiHandlerInterface
{
    ...
    {
        ActivityApiHandler::addProtectedRoutes($app);
    }

    public static function addProtectedRoutes(App $app)
    {

        $middleWare = $app->getContainer()->get(OAuth2MiddleWare::class);

        $app->group('/activities',function(RouteCollectorProxy $group){
            $group->get('',self::class.":handleActivityRequest")->setName('getActivities');

            $group->get('/{id}/details',self::class.":handleActivityRequest")->setName('getActivityDetails');
            $group->get('/{id}/summary',self::class.":handleActivityRequest")->setName('getActivitySummary');
            $group->get('/{id}/splits',self::class.":handleActivityRequest")->setName('getActivitySplits');
            $group->get('/{id}',self::class.":handleActivityRequest");
            $group->get('/compare/{ids}',self::class.":handleCompareRequest")->setName('getCompareMatrix');
        })->addMiddleware($middleWare);


        $app->get('/records',self::class.":handleRecordRequest")->addMiddleware($middleWare);
        $app->get('/summary',self::class.":handleSummaryRequest")->addMiddleware($middleWare);
        $app->get('/summary/extended',self::class.":handleSummaryRequest")->addMiddleware($middleWare);
    }
    ...
    public function handleActivityRequest(ServerRequest  $request, Response $response, array $args) : Response{
        if(array_key_exists('id',$args) && !empty($args['id'])){
            switch($args['id']) {
                ...
                case 'summary':
                    return $this->handleSummaryRequest($request,$response,$args);
                default:
    ...
    public function handleSummaryRequest(ServerRequest $request, Response $response, array $args): Response{
        ...
        if($request->getRequestTarget() === '/garmin/summary'){
            $response->getBody()->write(json_encode($summary));
        }else if($request->getRequestTarget() === '/garmin/summary/extended'){
