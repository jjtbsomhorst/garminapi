...
class GarminConnect
{
    ...
    public function proxy(string $host,$port) : GarminConnect{
        $this->objConnector->setProxy($host,$port);

        return $this;
    }

    public function jsonDecode(bool $decode) : GarminConnect{
        $this->decode = $decode;
        return $this;
    }
    ...
    public function getActivityTypes()
    {
        ...
        $objResponse = $this->doDecode($strResponse);
        return $objResponse;
    }

    public function getWorkouts($intStart = 0, $intLimit = 20){
        //https://connect.garmin.com/modern/proxy/workout-service/workouts?start=1&limit=999&myWorkoutsOnly=true&sharedWorkoutsOnly=false&orderBy=WORKOUT_NAME&orderSeq=ASC&includeAtp=false&_=1581876072590
        $strResponse = $this->objConnector->get('https://connect.garmin.com/modern/proxy/workout-service/workouts',array(
            'start'=>$intStart,
            'limit'=>$intLimit
        ));
        ...
        return $this->doDecode($strResponse);
    }

    private function doDecode(string $response){
        if($this->decode){
    ...
    public function getActivityCount(){
        $strResponse = $this->objConnector->get('https://connect.garmin.com/modern/proxy/activitylist-service/activities/count',[],true);
        if($this->objConnector->getLastResponseCode() != 200){
            throw new UnexpectedResponseCodeException($this->objConnector->getLastResponseCode());
        }
        return $this->doDecode($strResponse);
    ...
    /**
     * Gets the split information from this activity;
     * @param $intActivityID
     */
    public function getActivitySplits($intActivityID){
        $strResponse = $this->objConnector->get("https://connect.garmin.com/modern/proxy/activity-service/activity/" . $intActivityID."/splits");
        if ($this->objConnector->getLastResponseCode() != 200) {
            throw new UnexpectedResponseCodeException($this->objConnector->getLastResponseCode());
        }
        $objResponse = $this->doDecode($strResponse);
    ...
    public function getPersonalRecords(){
        ...
        $response = $this->objConnector->get(sprintf('https://connect.garmin.com/modern/proxy/personalrecord-service/personalrecord/prs/%s',$user->displayName));
        return $this->doDecode($response);
    }

    public function getHrZones(){
        $strResponse = $this->objConnector->get('https://connect.garmin.com/modern/proxy/biometric-service/heartRateZones');
